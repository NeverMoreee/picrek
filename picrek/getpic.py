...
def yan(url=yapi, limit=100, page=1, tags=None, proxy=None):
    ...
    logger.info('yandere:status_code:%s, page:%d, limit:%d', requests.status_code, page, limit)
    return req.text
...
def gel(url=gapi, limit=100, pid=1, tags=None, proxy=None):
    ...
    logger.info('gelbooru:status_code:%s, page:%d, limit:%d', requests.status_code, pid, limit)
    return req.text
